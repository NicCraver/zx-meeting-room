<template>
  <div class="m-app">
    <NavBarHeader @back="showToastSuccess('返回上一页（智信内嵌）')">
      <template #title>预定会议室</template>
      <template #actions>
        <button
          type="button"
          class="m-nav-icon"
          ...
          @click="showMore = true"
        >
          <SvgIcon name="more-point" class="w-22px h-22px text-ink" />
        </button>
      </template>
    </NavBarHeader>
    ...
    <CreateScheduleModal
      ...
      :date-iso="bookingRange.dateIso || boardDate"
...
import AiBuddyFab from "@/features/agent/components/AiBuddyFab.vue";
import "./booking.css";
...
const openBooking = (room, range) => {
  bookingRange.value = {
    ...range,
    dateIso: range.dateIso || boardDate.value
  };
...
const handleBookingSuccess = async (count = 1) => {
  const edited = Boolean(editingBooking.value);
  closeBooking();
...
const handleConfirmRelease = async () => {
  ...
  try {
    ...
    track("booking_release", { bookingId: booking.id });
...
onMounted(() => {
  track("page_view", { surface: "mobile" });
});
</script>
