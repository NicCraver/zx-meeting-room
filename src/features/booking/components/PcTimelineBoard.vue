        ...
        >
          ...
          >
            ...
            <div
              ...
              class="tl-slot-hover"
              :class="{ 'is-disabled': hoverHint.disabled }"
              :title="hoverHint.disabled ? '该时段不可预约' : '点击预约此时段'"
              :style="hoverHint.style"
            >
              <span
                v-if="!hoverHint.disabled"
                class="tl-slot-hover-plus"
                aria-hidden="true"
                >+</span
              >
            </div>
...
const hideDayNowLabel = computed(() => {
  const sel = props.selection;
  if (!sel || sel.view === "week") return false;
  return dayAxisNowHidden(nowMin.value, sel);
});
...
const visibleHours = computed(() =>
  TL.HOURS.filter(
    (h) =>
      !dayAxisHourHidden(h * 60, {
        nowMin: props.isToday ? nowMin.value : null,
        selection:
          props.selection && props.selection.view !== "week"
            ? props.selection
            : null
      })
  )
...
const onTrackHover = (room, e) => {
  ...
  if (isWeek.value) {
    ...
    const start = minute;
    const end = start + TL.SNAP;
    hoverHint.value = {
      roomId: room.id,
      disabled: !isBookableWeekMinute(room, dayIndex, start, {
  ...
  hoverHint.value = {
    roomId: room.id,
    disabled: !isBookableMinute(room, start, {
...
const handlePointerDown = (room, e) => {
  ...
  if (isWeek.value) {
    ...
    if (
      ...
      minute < TL.nextOpen(nowMin.value)
    ) {
      emit("notice", "该时段已过期");
      return;
    }
  ...
  if (props.isToday && anchor < TL.nextOpen(nowMin.value)) {
    emit("notice", "该时段已过期");
    return;
  }
