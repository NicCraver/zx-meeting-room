import { addDays, fromMinutes, toMinutes, TL } from "../booking/time.js";
import { draftFromGrid } from "../booking/bookingDefaults.js";
...
export const parseFindFreeQuery = (text, { todayIso, nowMin = 0 } = {}) => {
  ...
  if (hourDur)
    q.durationMin = Math.max(STEP, Math.round(Number(hourDur[1]) * 60));
  ...
  if (/视频/.test(raw) && !q.facilities.includes("电视"))
    q.facilities.push("电视");
...
const roomMatches = (room, query) => {
  ...
  if (
    query.buildingName &&
    !String(room.buildingName).includes(query.buildingName)
  ) {
    return false;
  }
  if (
    query.floorName &&
    !String(room.floorName).includes(query.floorName.replace("楼", ""))
  ) {
    return false;
  }
...
export const searchFreeSlots = (rooms, query, now) => {
  ...
  const { windowStart, windowEnd } = normalizeWindow(
    rawStart,
    rawEnd,
    durationMin
  );
