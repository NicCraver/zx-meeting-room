...
const TURN_URL = "/api/contact/v1/meetingRoom/agent/turn";
...
export async function streamTurn(body, onEvent, options = {}) {
  ...
  const turnUrl = new URL(TURN_URL, window.location.origin);
  const accountId = getAccountId();
  const corpId = getCorpId();
  if (accountId) turnUrl.searchParams.set("zxAccountId", accountId);
  if (corpId) turnUrl.searchParams.set("zxCorpId", corpId);
  turnUrl.searchParams.set("zxClientType", "app");

  const res = await fetch(`${turnUrl.pathname}${turnUrl.search}`, {
    method: "POST",
    headers,
